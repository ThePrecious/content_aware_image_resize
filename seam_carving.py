...
#From https://github.com/vivianhylee/seam-carving/blob/master/seam_carving.py with some modifications
import numpy as np
import cv2


class SeamCarver:
    def __init__(self, in_cv2_image, out_height, out_width, protect_mask='', object_mask=''):
        ...
        self.in_image = in_cv2_image #cv2.imread(filename).astype(np.float64)
        self.in_height, self.in_width = self.in_image.shape[: 2]

        # seam mask 
        self.seam_mask = np.zeros((self.in_height, self.in_width))
